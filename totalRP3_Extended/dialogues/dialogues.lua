...
local function loadScalingParameters(data, model, facing)
	-- The 3.3.5 compatibility scaling DB may not know this model.
...
local function modelsLoaded()
	if modelRight.modelLoaded and modelLeft.modelLoaded then
		...
		-- GetModelFileID is post-Wrath. 3.3.5 Model frames expose GetModel on
		-- supported clients; if neither accessor exists, advanced scaling is simply
		-- skipped while dialogue rendering continues.
		local function getModelIdentity(frame)
			local value;
			if frame.GetModelFileID then
				local ok, result = pcall(frame.GetModelFileID, frame);
				if ok then value = result; end
			elseif frame.GetModel then
				local ok, result = pcall(frame.GetModel, frame);
				if ok then value = result; end
			end
			return value and tostring(value) or nil;
		end

		modelLeft.model = getModelIdentity(modelLeft);
		modelRight.model = getModelIdentity(modelRight);
		...
		if modelLeft.model and dataLeft then
			loadScalingParameters(dataLeft, modelLeft, true);
		end

		-- Configuration for model right
		if modelRight.model and dataRight then
	...
	if not modelID or modelID:len() == 0 or (modelID == "target" and not UnitExists("target")) then
		...
		elseif modelID:sub(1, 3) == "DID" then
			...
			elseif model.SetCreature then
				model:SetCreature(tonumber(modelID:sub(4, modelID:len())) or 0);
			end
		else
			if model.SetCreature then model:SetCreature(tonumber(modelID) or 0); end
		end
		-- Wrath PlayerModel has no OnModelLoaded script. Model assignment is
		-- synchronous enough for Extended's presentation path; advanced scaling
		-- is guarded separately when a model identity is unavailable.
		model.modelLoaded = true;
	end
end
...
function TRP3_API.extended.dialog.onStart()
	...
	-- 3.3.5 PlayerModel does not implement the Retail OnModelLoaded script.
	-- setModel() marks each side ready and processDialogStep() calls modelsLoaded.

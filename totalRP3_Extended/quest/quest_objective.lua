				...
				if questLog.OB then
					...
					for _, objectiveID in pairs(objIds) do
						...
						if objectiveClass then
							...
							if state == true then
								objText = "|TInterface\\RAIDFRAME\\ReadyCheck-Ready:12:12|t " .. obectiveText;
							else
...
function frame.init()
	...
	local ticker = C_Timer.NewTicker(1, function()
		...
		-- Retail/Legion objective tracker path.
		if ObjectiveTrackerBlocksFrame and ObjectiveTrackerBlocksFrame.QuestHeader
				and ObjectiveTrackerBlocksFrame.QuestHeader.module
				and ObjectiveTrackerBlocksFrame.QuestHeader.module.lastBlock then
		...
		-- WotLK 3.3.5 quest tracker. Place Extended objectives beneath it.
		if WatchFrame then
			frame:SetPoint("TOPRIGHT", WatchFrame, "BOTTOMRIGHT", -5, -8);
			display();
			if frame.html and frame.html ~= "" then frame:Show(); end
		end
	end);
end